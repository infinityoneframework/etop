  ...
  describe "load" do
    ...
    test "print/1 entry" do
      expected = [
        ...
        "Load:  cpu     2.9%                      Memory:  total               40MB     binary     214KB",
        "       procs     92                               processes           21MB     code        10MB",
        "       runq       0                                atom              501KB      ets       779KB",
        "",
        "Pid                            Name or Initial Func  Percent          Reds    Memory MsgQ      State Current Function",
        "------------------------------------------------------------------------------------------------------------------------------------",
        "<0.9.0>                             :erlang.apply/2    29.28        921464     417KB    0    waiting :erl_prim_loader.loop/3",
        "<0.226.0>                      IEx.Evaluator.init/4    27.83        875941       1MB    0    waiting IEx.Evaluator.loop/1",
        "<0.49.0>                            :erlang.apply/2     7.69        241976     192KB   10    waiting :code_server.loop/1",
        "<0.64.0>                            :group.server/3     6.56        206407       8MB    0    waiting :group.more_data/6",
        "<0.62.0>                         :user_drv.server/2     5.46        172007     107KB    5    waiting :user_drv.server_loop/6",
        "<0.1.0>                   :erts_code_purger.start/0     5.22        164253      20KB    0    waiting :erts_code_purger.wait_for_request/0",
        "<0.2.0>         :erts_literal_area_collector.start/     5.01        157751       3KB    0    waiting :erts_literal_area_collector.msg_loop/4",
        "<0.43.0>            :application_controller.start/1     4.89        153876     258KB    0    waiting :gen_server.loop/7",
        "<0.57.0>                        :file_server.init/1     3.59        112924     417KB    4    waiting :gen_server.loop/7",
        "<0.79.0>                           :disk_log.init/2     1.22         38389     258KB    0    waiting :disk_log.loop/1",
        "====================================================================================================================================",
        "",
        "",
        ""
    ...
    test "print/1 entrys" do
      expected = [
        ...
        "Load:  cpu     1.5%                      Memory:  total               62MB     binary     248KB",
        "       procs     92                               processes           43MB     code        10MB",
        "       runq       0                                atom              501KB      ets       781KB",
        "",
        "Pid                            Name or Initial Func  Percent          Reds    Memory MsgQ      State Current Function",
        "------------------------------------------------------------------------------------------------------------------------------------",
        "<0.449.0>                               Etop.init/1     95.7        103956      29MB    0    running Process.info/1",
        "<0.9.0>                             :erlang.apply/2     3.44          3742     417KB    0    waiting :erl_prim_loader.loop/3",
        "<0.49.0>                            :erlang.apply/2     0.55           601     192KB    0    waiting :code_server.loop/1",
        "<0.57.0>                        :file_server.init/1     0.11           119     417KB    0    waiting :gen_server.loop/7",
        "<0.3.0>         :erts_dirty_process_signal_handler.     0.03            30       3KB    0    waiting :erts_dirty_process_signal_handler.msg_loop/0",
        "<0.79.0>                           :disk_log.init/2      0.0             4     258KB    0    waiting :disk_log.loop/1",
        "<0.46.0>             :application_master.start_it/4      0.0             2       7KB    0    waiting :application_master.loop_it/4",
        "<0.91.0>             :application_master.start_it/4      0.0             2       3KB    0    waiting :application_master.loop_it/4",
        "<0.115.0>       Mix.ProjectStack.-start_link/1-fun-      0.0             2      54KB    0    waiting :gen_server.loop/7",
        "<0.51.0>                                :rpc.init/1      0.0             2       3KB    0    waiting :gen_server.loop/7",
        "====================================================================================================================================",
        "",
        "",
        ""
    ...
    test "print/2 sort humanized" do
      expected = [
        "====================================================================================================================================",
        "nonode@nohost                                                                                                               08:36:11",
        "Load:  cpu     2.9%                      Memory:  total               40MB     binary     214KB",
      ...
      assert capture_io(fn ->
               @log_path |> Report.load() |> hd() |> Report.print(sort: :msgq, human: true)
             end)
             |> String.split("\n")
             |> Enum.map(&String.trim_trailing/1) == expected
    end
