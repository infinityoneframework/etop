defmodule Etop.WatcherTest do
  ...
  test "add_monitors" do
    ...
    assert Etop.monitors() == [
      {:process, :message_queue_len, msgq_fn, {Etop.WatcherTest.WatcherServer, :message_queue_callback}},
      {:summary, [:load, :total], load_fn, {Etop.WatcherTest.WatcherServer, :load_callback}}
    ]
  end
  ...
  test "trigger monitors" do
    ...
    capture_io(fn ->
      logs = capture_log(fn ->
        ...
        Process.sleep(200)
      end) |> String.split("\n", trim: true) |> Enum.filter(& &1 =~ "[info]") |> Enum.take(3)
  ...
  describe "handle_call :load_callback" do
    ...
    test ">= enable_limit", %{state: state} do
      assert capture_log(fn ->
        ...
        {:reply, reply, _} = nil |> load_callback_params(90, etop) |> Server.handle_call(:dc, state)
    ...
    test "<= notify_lower_limit", %{state: state} do
      ...
      assert capture_log(fn ->
        etop = %{reporting: true}
        {:reply, reply, state1} = nil |> load_callback_params(49, etop) |> Server.handle_call(:dc, state)
        assert reply.reporting
      ...
      assert capture_log(fn ->
        etop = %{reporting: true}
        {:reply, reply, state1} = nil |> load_callback_params(49, etop) |> Server.handle_call(:dc, orig_state)
        assert reply.reporting
    ...
    test "<= disable_limit", %{state: state} do
      ...
      assert capture_log(fn ->
        etop = %{reporting: true}
        {:reply, reply, state1} = nil |> load_callback_params(9, etop) |> Server.handle_call(:dc, state)
      ...
      assert capture_log(fn ->
        etop = %{reporting: true}
        {:reply, reply, state1} = nil |> load_callback_params(9, etop) |> Server.handle_call(:dc, orig_state)
        refute reply.reporting
        assert state1 == orig_state
      end) == ""
    ...
    test "default", %{state: state} do
      assert capture_log(fn ->
        etop = %{reporting: false}
        {:reply, reply, state1} = nil |> load_callback_params(60, etop) |> Server.handle_call(:dc, state)
        assert reply == etop
        assert state1 == state
      end) == ""
      ...
      assert capture_log(fn ->
      etop = %{reporting: true}
        {:reply, reply, state1} = :dc |> load_callback_params(10.0, etop) |> Server.handle_call(:dc, state)
        refute reply.reporting
        assert state1 == state
      end) == ""
  ...
  describe "handle_call :message_queue_callback" do
    ...
    test ">= stop_limit", %{state: state} do
      ...
      assert capture_log(fn ->
        etop = %{reporting: false}
        {:reply, reply, _} = %{pid: pid} |> queue_callback_params(20_000, etop) |> Server.handle_call(:dc, state)
        refute reply.reporting
    ...
    test ">= notify_limit", %{state: state} do
      ...
      assert capture_log(fn ->
        etop = %{reporting: false}
        {:reply, reply, state1} = %{pid: pid} |> queue_callback_params(2000, etop) |> Server.handle_call(:dc, state)
        assert reply.reporting
    ...
    @tag opts: [notify_log: true]
    test "<= lower_limit", %{state: _state} do
    end
  end
